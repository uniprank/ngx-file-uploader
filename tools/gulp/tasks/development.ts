...
import {DIST_ROOT, SOURCE_ROOT, PROJECT_ROOT, DIST_BUNDLES, DIST_THISAPP} from '../constants';
import {
...
task('serve:devapp', ['build:devapp'], sequenceTask(
  [':serve:devapp', 'ngxfileuploader:watch', ':watch:devapp']
));
...
task('stage-deploy:devapp', ['build:devapp'], () => {
  ...
  copyFiles(DIST_THISAPP, '**/prebuilt/*.+(css|map)', join(outDir, 'dist/packages/ngxfileuploader'));
});
