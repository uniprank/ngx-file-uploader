...
function uploadScreenshotsData(database: firebase.database.Database,
  ...
  const promises = getLocalScreenshotFiles(localDir).map((file: string) => {
    const fileName = path.join(localDir, file);
    const filenameKey = extractScreenshotName(fileName);
    const secureToken = getSecureToken();
...
function diffScreenshot(filename: string, database: firebase.database.Database,
  ...
  const goldUrl = path.join(LOCAL_GOLDENS, filename);
  const pullRequestUrl = path.join(SCREENSHOT_DIR, filename);
  const diffUrl = path.join(LOCAL_DIFFS, filename);
  const filenameKey = extractScreenshotName(filename);
...
function uploadScreenshots() {
  ...
  const promises = getLocalScreenshotFiles(SCREENSHOT_DIR).map((file: string) => {
    const fileName = path.join(SCREENSHOT_DIR, file);
    const destination = `${FIREBASE_STORAGE_GOLDENS}/${file}`;
