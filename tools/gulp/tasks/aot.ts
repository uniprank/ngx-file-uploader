...
/** Builds the demo-app and ngxfileuploader. To be able to run NGC, apply the metadata workaround. */
task('aot:deps', sequenceTask(
  'build:devapp',
  ['ngxfileuploader:build-release', 'cdk:build-release'],
  'aot:copy-release'
));
...
task('aot:copy-release', () => {
  copySync(join(DIST_RELEASES, 'ngxfileuploader'), join(DIST_DEMOAPP, 'ngxfileuploader'));
