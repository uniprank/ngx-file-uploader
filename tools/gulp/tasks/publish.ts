...
function _execNpmPublish(label: string, access: string, packageName: string): Promise<{}> {
  ...
  const command = 'npm';
  const args = ['publish', '--access', access, label ? `--tag` : undefined, label || undefined];
...
task(':publish', async () => {
  ...
  const access = argv['access'] || 'restricted';
  ...
  console.log(yellow(`Publishing using the ${access} access.`));
  console.log('');
  ...
  for (const packageName of releasePackages) {
    await _execNpmPublish(label, access, packageName);
  }
