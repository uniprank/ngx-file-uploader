...
// `ngxfileuploader-docs-example` attribute which can be used to locate the divs and initialize the example
// viewer.
...
task('highlight-examples', () => {
  ...
  return src('src/ngxfileuploader-examples/**/*.+(html|css|ts)')
      .pipe(flatten())
...
task('plunker-example-assets', () => {
  src(path.join(SOURCE_ROOT, 'ngxfileuploader-examples', '**/*'))
...
function transformMarkdownFiles(buffer: Buffer, file: any): string {
  ...
  content = content.replace(EXAMPLE_PATTERN, (match: string, name: string) =>
    `<div ngxfileuploader-docs-example="${name}"></div>`
  );
...
/** Fixes paths in the markdown files to work in the ngxfileuploader-docs-io. */
