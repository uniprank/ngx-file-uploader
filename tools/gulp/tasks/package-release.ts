...
import {COMPONENTS_DIR, DIST_THISAPP, DIST_RELEASES} from '../constants';

// There are no type definitions available for these imports.
const gulpRename = require('gulp-rename');

// Path to the release output of ngxfileuploader.
const releasePath = join(DIST_RELEASES, 'ngxfileuploader');
...
const prebuiltThemeGlob = join(DIST_THISAPP, '**/theming/prebuilt/*.css?(.map)');

// Matches all SCSS files in the library.
const allScssGlob = join(COMPONENTS_DIR, '**/*.scss');

/**
 * Overwrite the release task for the ngxfileuploader package. The ngxfileuploader release will include special
 * files, like a bundled theming SCSS file or all prebuilt themes.
 */
task('ngxfileuploader:build-release', ['ngxfileuploader:prepare-release'], () => composeRelease('ngxfileuploader'));

/**
 * Task that will build the ngxfileuploader package. It will also copy all prebuilt themes and build
 * a bundled SCSS file for theming
 */
task('ngxfileuploader:prepare-release', sequenceTask(
  'ngxfileuploader:build'
  // , ['ngxfileuploader:copy-prebuilt-themes', 'ngxfileuploader:bundle-theming-scss']
));

/** Copies all prebuilt themes into the release package under `prebuilt-themes/` */
task('ngxfileuploader:copy-prebuilt-themes', () => {
...
task('ngxfileuploader:bundle-theming-scss', () => {
