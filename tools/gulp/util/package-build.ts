...
import {
  DIST_BUNDLES, DIST_ROOT, SOURCE_ROOT, PROJECT_ROOT, LICENSE_BANNER, PACKAGE_VERSION
} from '../constants';
...
export function composeRelease(packageName: string) {
  // To avoid refactoring of the project the package ngxfileuploader will map to the source path `lib/`.
  const sourcePath = join(SOURCE_ROOT, packageName === 'ngxfileuploader' ? 'lib' : packageName);
  ...
  copyFiles(DIST_BUNDLES, `${packageName}?(.es5).js?(.map)`, join(releasePath, '@uniprank'));
...
export async function buildPackageBundles(entryFile: string, packageName: string) {
  const moduleName = `ng.ngxfileuploader.${packageName}`;
...
function updatePackageVersion(packageDir: string) {
  ...
  packageConfig.version = packageConfig.version.replace('0.0.0-PLACEHOLDER', PACKAGE_VERSION);
...
function addPureAnnotationCommentsToEs5Bundle(outputDir: string, entryName: string) {
  const es5BundlePath = join(outputDir, '@uniprank', `${entryName}.es5.js`);
