...
export function openFirebaseDashboardDatabase() {
  ...
  firebaseAdmin.initializeApp({
    credential: firebaseAdmin.credential.cert({
      project_id: 'ngxfileuploader-dashboard',
      client_email: 'firebase-adminsdk-ch1ob@ngxfileuploader-dashboard.iam.gserviceaccount.com',
      // In Travis CI the private key will be incorrect because the line-breaks are escaped.
      // The line-breaks need to persist in the service account private key.
      private_key: decode(process.env['MATERIAL2_FIREBASE_PRIVATE_KEY'])
    }),
    databaseURL: 'https://ngxfileuploader-dashboard.firebaseio.com'
  });
...
export function openScreenshotsBucket() {
  const gcs = cloudStorage({
    projectId: 'ngxfileuploader-screenshots',
    credentials: {
      client_email: 'firebase-adminsdk-t4209@ngxfileuploader-screenshots.iam.gserviceaccount.com',
      private_key: decode(process.env['MATERIAL2_SCREENSHOT_FIREBASE_KEY'])
    },
  });

  // Reference the existing appspot bucket.
  return gcs.bucket('ngxfileuploader-screenshots.appspot.com');
}
